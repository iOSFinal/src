...
-(void)loadDataById:(NSString*)Id{
    ...
    [query includeKey:@"headImg"];
    NSArray *a=[query findObjects];
    AVObject *avobj=a[0];
    NSString *username=avobj[@"username"];
    NSString *sex=avobj[@"sex"];
    NSString *address=avobj[@"address"];
    NSString *birthday=avobj[@"birthday"];
    NSString *qianming=avobj[@"qianming"];
    ...
    [file downloadWithProgress:^(NSInteger number) {
        ...
        [self set_dataByimg:i username:username sex:sex birthday:birthday address:address qianming:qianming];
    }];
}
...
-(void)load_userHeadImg:(UIImage*)img username:(NSString*)username info:(NSString*)info{
    ...
    UILabel *name=[[UILabel alloc]initWithFrame:CGRectMake(20+img_size+20, headImg_height/2-img_size/2, 150, 30)];
    name.text=username;
    [name setFont:[UIFont fontWithName:@"Arial" size:24.0]];
    [view addSubview:name];
    
    UILabel *qianming=[[UILabel alloc]initWithFrame:CGRectMake(20+img_size+20, headImg_height/2-img_size/2+30+10, 250, 90)];
    qianming.text=info;
