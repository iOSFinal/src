...
-(void)load_view{
    ...
    _passwd.keyboardType = UIKeyboardTypeASCIICapable;
    [_passwd addTarget:self action:@selector(putinEvent:) forControlEvents:UIControlEventEditingChanged];
    ...
    _passwd_appead.clearButtonMode = UITextFieldViewModeWhileEditing;
    _passwd_appead.keyboardType = UIKeyboardTypeASCIICapable;
...
-(void)putinEvent:(UITextField*)text{
...
-(BOOL)checkinput{
    if(_username.text.length==0){
        ...
        LKErrorBubble(@"用户名不能为空", 2);
        return false;
    }
    else if(_username.text.length>10){
        LKErrorBubble(@"用户名过长", 2);
        return false;
    }
    else if(_passwd.text != _passwd_appead.text){
        //[self showwindow:@"重复密码错误"];
        LKErrorBubble(@"重复密码错误", 2);
        return false;
    }
    else if(_passwd.text.length==0){
        //[self showwindow:@"密码不能为空"];
        LKErrorBubble(@"密码不能为空", 2);
        return false;
    }
    else if(_passwd.text.length>20){
        LKErrorBubble(@"密码长度过长", 2);
        return false;
    }
    return true;
}
...
-(void)signupOver{
    ...
    if([self checkinput]==true){
        ...
        [user setObject:@"不明" forKey:@"sex"];         //初始化性别
        [user setObject:@"2000年1月1日" forKey:@"birthday"];    //生日
        [user setObject:@"" forKey:@"address"];     //地址
        [user setObject:@"" forKey:@"qianming"];    //签名
        
        UIImage *headimg=[UIImage imageNamed:@"yonghu.png"];
        NSData *headdata=UIImageJPEGRepresentation(headimg, 1.0);
        AVFile *file=[AVFile fileWithData:headdata name:@"yonghu.png"];
        [user addObject:file forKey:@"headImg"];
        //[user addObject:file forKey:@"headImg"];
